...
def getFilePic(picname):
    Inputimg = cv2.imread('E:\\cuiyuhao\\python\\SemanticSegmentation_AICAR\\Predict\\' + picname + '.png')
...
Data_batch,Label_batch=getBatchPic('E:\\cuiyuhao\\python\\dataset\\Cityscapes\\BGR512\\Cityscapes512_00.h5',
                                   900,910,getlabel=DrawLabelPic)
...
L4=Dconv_block(Inputtensor=L3_8,Inputchannel=128,Outputchannel=64,output_shape=[Data_batch.shape[0],64,128,64])
L4_1=Res_block(Inputtensor=L4,channel=64,dilatedRate=1,residualNum=2,istrain=BNtrain,dropoutKeepProb=keep_prob)
L5=Dconv_block(Inputtensor=L4_1,Inputchannel=64,Outputchannel=16,output_shape=[Data_batch.shape[0],128,256,16])
L5_1=Res_block(Inputtensor=L5,channel=16,dilatedRate=1,residualNum=2,istrain=BNtrain,dropoutKeepProb=keep_prob)
L6=Dconv_block(Inputtensor=L5_1,Inputchannel=16,Outputchannel=29,output_shape=[Data_batch.shape[0],256,512,3])
...
for picnum in range(PredictVector.shape[0]):
    ...
    cv2.imwrite('E:\\cuiyuhao\\python\\SemanticSegmentation_AICAR\\Predict\\' + str(picnum) + '_SemanticSeg.jpg',PredictPic)
    ...
    if DrawLabelPic:
        ...
        cv2.imwrite('E:\\cuiyuhao\\python\\SemanticSegmentation_AICAR\\Predict\\' + str(picnum) + '_Label.jpg',LabelPic)

    if DrawOriginPic:
        cv2.imwrite('E:\\cuiyuhao\\python\\SemanticSegmentation_AICAR\\Predict\\' + str(picnum) + '_Origin.jpg',Data_batch[picnum])
